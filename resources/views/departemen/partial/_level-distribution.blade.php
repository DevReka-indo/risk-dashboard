        ...
        @forelse ($dashboardData['level_distribution'] as $level)
            <div>
                ...
                <div class="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
                    <div class="h-full rounded-full bg-indigo-600 transition-all duration-500 ease-in-out" style="width: {{ $level['percentage'] }}%"></div>
                </div>
            </div>
        @empty
            <div class="text-center text-sm text-slate-400 py-4">Tidak ada data level risiko.</div>
        @endforelse
    </div>
</div>
