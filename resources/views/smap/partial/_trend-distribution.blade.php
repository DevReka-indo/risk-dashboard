<div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
    <div class="mb-4">
        <h3 class="text-sm font-bold text-slate-900">
            Distribusi Trend Risiko
        </h3>
        <p class="text-xs text-slate-500">
            Sebaran risiko SMAP berdasarkan trend.
        </p>
    </div>
    ...
    <div class="space-y-3">
        @forelse ($items as $item)
            <div class="flex items-center gap-3">
                <span class="inline-flex min-w-[120px] rounded-full px-3 py-1 text-xs font-semibold {{ $item['color'] ?? 'bg-slate-100 text-slate-600' }}">
        ...
        @empty
            <p class="text-sm text-slate-400">Belum ada data trend risiko.</p>
        @endforelse
    </div>
</div>