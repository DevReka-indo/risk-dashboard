<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-base font-bold text-slate-900">Edit Risk SMAP</h1>
                <p class="text-xs text-slate-500">Perbarui data risiko SMAP di dalam sistem</p>
            </div>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('smap-risk.update', $risk->id_smap) }}" x-data="smapRiskForm({}, '', '{{ old('inherent', $risk->inherent) }}', '{{ old('inherent_target', $risk->inherent_target) }}')">
        @csrf
        @method('PUT')
                        ...
                        <div style="display:flex; flex-direction:column; gap:18px;">
                            ...
                            <div>
                                ...
                                <input type="date" name="created_at"
                                       value="{{ old('created_at', \Carbon\Carbon::parse($risk->created_at)->format('Y-m-d')) }}"
                                ...
                                <div style="border:1px solid #e2e8f0; border-radius:10px; padding:12px; background:#fff;">
                                    <div style="display:flex; flex-direction:column; gap:8px;">
                                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                            <input type="radio" name="status" value="1" style="accent-color:#4F7EF0;" @checked(old('status', $risk->status) == '1')>
                                            <span style="font-size:13px; color:#475569;">Aktif</span>
                                        </label>
                                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                            <input type="radio" name="status" value="0" style="accent-color:#4F7EF0;" @checked(old('status', $risk->status) == '0')>
                    ...
                    <td style="padding:24px; vertical-align:top;">
                        <div style="display:flex; flex-direction:column; height:100%;">
                            ...
                            <textarea name="risk_event_deta" placeholder="Masukkan peristiwa resiko secara detail..."
                                      style="flex:1; width:100%; border:1px solid #e2e8f0; border-radius:10px; padding:12px; font-size:13px; color:#475569; background:#fff; resize:none; outline:none; min-height:240px; font-family:inherit; line-height:1.6;">{{ old('risk_event_deta', $risk->risk_event_deta) }}</textarea>
        ...
        {{-- TOMBOL AKSI DITAMBAHKAN DI DALAM FORM SINI --}}
    ...
    </form> {{-- 👈 PENUTUP FORM YANG BENAR ADA DI SINI --}}
