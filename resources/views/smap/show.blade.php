                ...
                <div style="display:grid; grid-template-columns:200px 1fr; gap:20px; align-items:start;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:16px;">
                        ...
                        <div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:14px;">
                            ...
                            <div>
                                <label style="display:block; font-size:12px; font-weight:700; color:#1e293b; margin-bottom:6px;">Progress Belum</label>
                                <input type="number" name="progress_belum" value="{{ old('progress_belum', 0) }}" min="0"
                                       style="width:100%; border:1px solid #e2e8f0; border-radius:10px; padding:9px 12px; font-size:13px; color:#475569; background:#fff; outline:none; box-sizing:border-box; height:38px;">
                            </div>
                            <div>
                                <label style="display:block; font-size:12px; font-weight:700; color:#1e293b; margin-bottom:6px;">Progress Proses</label>
                                <input type="number" name="progress_proses" value="{{ old('progress_proses', 0) }}" min="0"
                                       style="width:100%; border:1px solid #e2e8f0; border-radius:10px; padding:9px 12px; font-size:13px; color:#475569; background:#fff; outline:none; box-sizing:border-box; height:38px;">
                            </div>
                            <div>
                                <label style="display:block; font-size:12px; font-weight:700; color:#1e293b; margin-bottom:6px;">Progress Sudah</label>
                                <input type="number" name="progress_sudah" value="{{ old('progress_sudah', 0) }}" min="0"
                                       style="width:100%; border:1px solid #e2e8f0; border-radius:10px; padding:9px 12px; font-size:13px; color:#475569; background:#fff; outline:none; box-sizing:border-box; height:38px;">
                            </div>
                        </div>

                        {{-- Baris 3: Status Monitoring + Level saat ini + Trend --}}
                ...
                @forelse($risk->detailPeriode as $history)
                    ...
                    <div x-data="{ editOpen: false }"
                        ...
                        <div style="padding:12px 20px; border-bottom:1px solid #f1f5f9;">
                            <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
                                <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                                    ...
                                    <span style="background:#f1f5f9; color:#475569; border-radius:20px; padding:4px 12px; font-size:12px; font-weight:600;">
                                        Belum: {{ $history->progress_belum ?? 0 }} | Proses: {{ $history->progress_proses ?? 0 }} | Sudah: {{ $history->progress_sudah ?? 0 }}
                                    </span>
                        ...
                        <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; padding:16px 20px;">
                            ...
                            {{-- Kotak 3: Progress Belum / Proses / Sudah --}}
                            <div style="border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; background:#fafbfc;">
                                <p style="font-size:10px; font-weight:600; color:#94a3b8; margin:0 0 4px; text-transform:uppercase; letter-spacing:0.3px;">Progress Penanganan</p>
                        ...
                        <div x-show="editOpen" x-transition style="display:none; border-top:1px solid #f1f5f9; padding:16px 20px; background:#fafbfc;">
                            <form method="POST" action="{{ route('smap-risk.update-monitoring', $history->id_detail) }}">
                                ...
                                <div style="display:grid; grid-template-columns:repeat(7,1fr); gap:12px; margin-bottom:12px;">
                                    ...
                                    <div>
                                        <label style="font-size:12px; font-weight:600; color:#1e293b; display:block; margin-bottom:5px;">Progress Belum</label>
                                        <input type="number" name="progress_belum" min="0"
                                            value="{{ old('progress_belum', $history->progress_belum ?? 0) }}"
                                    ...
                                    <div>
                                        <label style="font-size:12px; font-weight:600; color:#1e293b; display:block; margin-bottom:5px;">Progress Proses</label>
                                        <input type="number" name="progress_proses" min="0"
                                            value="{{ old('progress_proses', $history->progress_proses ?? 0) }}"
                                            style="width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:8px 10px; font-size:13px; outline:none; box-sizing:border-box; background:#fff;">
                                    </div>
                                    <div>
                                        <label style="font-size:12px; font-weight:600; color:#1e293b; display:block; margin-bottom:5px;">Progress Sudah</label>
                                        <input type="number" name="progress_sudah" min="0"
                                            value="{{ old('progress_sudah', $history->progress_sudah ?? 0) }}"
                                            style="width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:8px 10px; font-size:13px; outline:none; box-sizing:border-box; background:#fff;">
                                    </div>
                                    <div>
